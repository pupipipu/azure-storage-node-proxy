        ...
        if(self._maximumExecutionTimeExceeded(Date.now(), options.operationExpiryTime)) {
          ...
          var buildRequest = function (headersOnly, inputStream) {
            ...
            if(self.proxy) {
              ...
              //try to add the proxy directly to the requestOptions
              if(finalRequestOptions){
                finalRequestOptions.proxy = self.proxy.protocol + '//' + self.proxy.host + ':' + self.proxy.port;
              }
